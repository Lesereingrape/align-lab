"""A tiny decoder-only transformer with the sequence logprob used by alignment.

Same nanoGPT-shaped architecture the STaR lab uses (learned token + position
embeddings, a couple of causal attention/MLP blocks, a linear head) at a few
tens of thousands of parameters, so the whole study runs in CPU seconds.

On top of the SFT cross-entropy we expose ``seq_logprob``: the teacher-forced
log probability of a CoT continuation given its prompt. DPO, ORPO and SimPO are
all defined in terms of that quantity (and, for DPO, a frozen reference copy of
it), so it is the shared primitive behind the comparison.
"""
